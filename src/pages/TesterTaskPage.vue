<template>
  <q-page padding>
    <div class="header-section">
      <h4>Bug List</h4>
      <q-btn icon="add" color="primary" round @click="showAddBugDialog = true" />
    </div>
    <div class="buttons">
...
const showAddBugDialog = ref(false); // State for the add bug dialog
...
const newBug = ref({ title: '', description: '', priority: 'Low' }); // State for new bug
...
function addNewBug() {
  if (newBug.value.title && newBug.value.description) {
    const bug = { 
      ...newBug.value, 
      bugid: Date.now().toString(), 
      completed: false, 
      assignedTo: '',
      dueDate: null,
      comment: ''
    ...
    bugStore.addBug(bug);
    newBug.value = { title: '', description: '', priority: 'Low' }; // Reset form
    showAddBugDialog.value = false; // Close the dialog
  }
}
